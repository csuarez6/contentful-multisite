...
const TabsWithFeaturedImageBlock: React.FC<IPromoBlock> = ({
  title,
  description,
  image,
  ...
  useEffect(() => {
    const content: IPromoContent = featuredContentsCollection?.items?.[0];
    setCurrentImage(content?.promoImage ?? image);
            ...
            <div className="grid gap-6 grow w-full 2md:w-1/2 2md:py-6">
              {(title || description) && (
                <div className="grid gap-6">
                  {title && (
                    <h2 className="text-blue-dark 2md:text-category-sky-blue-50 text-center 2md:text-left text-size-subtitle1 2md:text-[43px] leading-[1.3] font-bold">
                      {title}
                    </h2>
                  )}
                  {description && (
              ...
              {featuredContentsCollection?.items?.length > 0 && (
                <Tab.Group
                  as="div"
                  className="grid items-baseline px-3 2md:p-0 gap-6 2md:gap-12"
                >
                      ...
                      <Tab.List className="flex">
                        {featuredContentsCollection.items.map((tab: IPromoContent) => (
                          <Tab
                            ...
                            onClick={() => setCurrentImage(tab?.promoImage)}
                          >
                            {({ selected }) => (
                              <>
                      ...
                      >
                        ...
                        >
                          <div className="flex flex-col gap-4">
                            <h3 className="text-category-sky-blue-50 2md:text-blue-dark text-size-subtitle2 2md:text-2xl 2md:leading-7">
                              {content.subtitle ??
                                content.promoTitle ??
                                content.name}
                            </h3>
              ...
              >
                <figure className="relative rounded-xl w-full aspect-[494/500] overflow-hidden">
                  <Image
                    ...
                    alt={currentImage.title ?? currentImage.description ?? "Imagen destacada"}
                    width={494}
                    height={500}
                    className="object-cover w-full h-full"
                  />
