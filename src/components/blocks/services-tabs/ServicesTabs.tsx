...
const TabElement = (tab) => {
  return <>
    ...
    <p className='font-semibold leading-[1.6]'>{tab.promoTitle ?? tab.name}</p>
  </>
}

const ServicesTabsBlock: React.FC<IPromoBlock> = ({ title, featuredContentsCollection }) => {
  return (
      ...
      {featuredContentsCollection?.items && (
        <Tab.Group as="div" className="mt-4">
              ...
              <Tab.List className="flex gap-[10px]">
                {
                  featuredContentsCollection.items.map((tab) => {
                    return tab.internalLink?.slug || tab.externalLink
                      ? (
                        <Link href={tab.externalLink ?? tab.internalLink.slug} className="flex" key={tab.name}>
                          <a
          ...
          <Tab.Panels as={Fragment}>
            {featuredContentsCollection.items.map((tab) => (
              (!tab.internalLink && !tab.externalLink) && (
