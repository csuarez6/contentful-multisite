    ...
    return (
        ...
        >
            {data?.map((slide, index) => {
                ...
                return (
                    <SwiperSlide key={index} className="relative max-h-[568px] min-h-[350px] 2md:min-h-fit">
                        <Image src={slide?.urlImage} alt="slider" className='h-[350px] 2md:h-auto object-cover' width={1920} height={568} priority />
                            ...
                            }
                            {
                                !slide?.link?.isExternal ?
                                    (<Link href={slide?.link?.href}>
                                        <a className={`self-start ${slide?.link?.classes ?? ''} font-semibold py-[9px] px-4 button text-xs md:text-base`} onClick={slide?.link?.onClick}>
                                            {slide?.link?.name}
                                        </a>
                                    </Link>) :
                                    (<a href={slide?.link?.href} className={`self-start ${slide?.link?.classes ?? ''} font-semibold py-[9px] px-4 button text-xs md:text-base`} onClick={slide?.link?.onClick}>
                                        {slide?.link?.name}
                                    </a>)
                            }
                        </div>
            ...
            <button className='cursor-pointer z-20 absolute xl:ml-[120px] ml-5 bottom-5 xl:bottom-10 hidden md:block' onClick={() => setstopSliderRotation(!stopSliderRotation)}>
                <Image
                    alt='stop/pause'
