            ...
            <tbody>
              {products.map((product) => (
                <tr key={product.id} className="border-b border-grey-60">
                  ...
                  <td className="flex justify-center px-3 py-4">
                    <div className="w-32 custom-number-input h-9">
                      <div className="relative flex flex-row w-full h-full mt-1 bg-transparent rounded-lg">
                        <button data-action="decrement" className="w-20 h-full border border-r-0 outline-none cursor-pointer rounded-l-3xl"
                          onClick={() => updateItemQuantity(product?.sku_code, product?.quantity - 1)} disabled={order?.skus_count < 2}
                        >
                          <span className="m-auto">−</span>
                        </button>
                        <input type="text" className="flex items-center w-full text-center outline-none border-y focus:outline-none text-md md:text-basecursor-default" name="custom-input-number" value={order?.skus_count} ></input>
                        <button data-action="increment" className="w-20 h-full border border-l-0 cursor-pointer rounded-r-3xl"
                  ...
                  <td className="hidden py-4 pl-3 pr-4 text-sm text-right text-blue-dark sm:pr-6 md:pr-0 sm:table-cell">{product.formatted_total_amount}</td>
                </tr>
              ))}
            </tbody>
          </table>
